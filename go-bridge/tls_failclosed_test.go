...
func TestResolveTailscaleRemote_NoTailscaleIP(t *testing.T) {
	d := resolveTailscaleRemote("", 8778, 8777, false, nil)
...
func TestResolveTailscaleRemote_ProductModeTLSDisabledNoPlaintext(t *testing.T) {
	d := resolveTailscaleRemote("100.64.1.2", 0, 8777, false, nil)
	if d.tailscaleURL != "" {
...
//
// dataDir=nil：退化为一次性 generateSelfSignedCert，不派生 pin（开发/测试路径）。
func TestResolveTailscaleRemote_HappyPathProducesWSS(t *testing.T) {
	d := resolveTailscaleRemote("100.64.1.2", 8778, 8777, false, nil)
	...
	if d.tlsPin != nil {
		t.Fatalf("dataDir=nil 不应派生 pin, got %+v", d.tlsPin)
	}
}
...
func TestResolveTailscaleRemote_DevModeTLSDisabledAllowsPlaintext(t *testing.T) {
	d := resolveTailscaleRemote("100.64.1.2", 0, 8777, true, nil)
