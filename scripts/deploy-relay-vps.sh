...
#   CORDCODE_RELAY_VPS_HOST, CORDCODE_RELAY_VPS_USER, CORDCODE_RELAY_VPS_PASS
...
REMOTE_BIN=/opt/cordcode-relay/bin/relay-server
LOCAL_BIN="${1:-/tmp/cordcode-relay-server}"

# 加载凭据（新 shell 可能未 source ~/.zshrc）
# shellcheck disable=SC1091
[ -n "${CORDCODE_RELAY_VPS_PASS:-}" ] || { [ -f ~/.zshrc ] && source ~/.zshrc 2>/dev/null || true; }
: "${CORDCODE_RELAY_VPS_PASS:?CORDCODE_RELAY_VPS_PASS 未设置（检查 ~/.zshrc）}"
export SSHPASS="$CORDCODE_RELAY_VPS_PASS"
...
echo "目标: $SSH_HOST ($CORDCODE_RELAY_VPS_USER@${CORDCODE_RELAY_VPS_HOST:-<via alias>}) → $REMOTE_BIN"
echo
...
ssh_retry '
  ...
  systemctl status cordcode-relay --no-pager 2>&1 | head -4
' || exit 1
...
echo
echo "==> 5/6 重启 cordcode-relay"
OLD_PID=$(ssh_retry 'systemctl show -p MainPID --value cordcode-relay')
ssh_retry 'systemctl restart cordcode-relay' || exit 1
sleep 3

echo
echo "==> 6/6 健康检查"
ssh_retry "
  NEW_PID=\$(systemctl show -p MainPID --value cordcode-relay)
  echo \"PID: \$OLD_PID → \$NEW_PID\"
  [ \"\$NEW_PID\" != \"$OLD_PID\" ] || { echo '⚠️  PID 未变，restart 可能未生效，检查 systemctl status'; }
  systemctl is-active cordcode-relay
  echo -n 'readyz:  '; curl -s http://127.0.0.1:8780/readyz; echo
  echo -n 'healthz: '; curl -s http://127.0.0.1:8780/healthz; echo
  echo '最近日志:'; journalctl -u cordcode-relay --no-pager -n 8 --since '30 seconds ago'
" OLD_PID="$OLD_PID" || exit 1

echo
echo "✅ 部署完成。"
echo "回滚：ssh $SSH_HOST 'mv $REMOTE_BIN.bak.$TS $REMOTE_BIN && systemctl restart cordcode-relay'"
echo
