...
// KnownAgentSessionIDs returns the set of agent session IDs tracked by cccode-macbridge.
// This is used to filter agent.ListSessions() output to only sessions owned by
// cccode-macbridge, excluding sessions created by external CLI usage in the same work_dir.
